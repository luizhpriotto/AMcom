       ...
       // }
        ...
        stage('Publishing Service...'){
                steps {
                    script {
                        if (env.SCOPE == 'prd'){
                                sh "docker service update shark_${SCOPE} --image ${registry}/shark:${SCOPE}${BUILD_NUMBER} --with-registry-auth"
                            }  
                        else{
                            ...
                            if (env.RELEASE_QAS == 'yes') {
                                echo "updating..."
                                sh "docker service create shark_${SCOPE} --with-registry-auth ${registry}/shark:${SCOPE}${BUILD_NUMBER}"
                            }
                            else{
                                echo "creating.."
